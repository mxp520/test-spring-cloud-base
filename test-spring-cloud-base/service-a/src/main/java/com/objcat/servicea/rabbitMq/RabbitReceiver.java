...
public class RabbitReceiver {
    ...
    public void cfgUserReceiveDealy(StudentEntity entity, Message message, Channel channel) throws IOException {
        System.out.println("===============接收队列接收消息====================");
        System.out.println("接收时间:{},接受内容:{}"+":"+ entity.toString());
        //通知 MQ 消息已被接收,可以ACK(从队列中删除)了
        channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
        try {
            System.out.println("消费成功:"+new Date());
        } catch (Exception e) {
